...
func (n *NodeStatusController) checkCloud() {
	...
	klog.V(5).Infof("node status changed ready: %v network unavailabe: %v",
	...
	node := n.createNode()
	if node != nil {
		klog.V(2).Infof("updating node spec %+v status %+v", node.Spec, node.Status)
		n.cb(node)
	}
}
...
func (n *NodeStatusController) Dump() []byte {
	dumpStruct := struct {
		...
		ControlLoopTimer:   n.controlLoopTimer,
	}
...
func (n *NodeStatusController) capacity() corev1.ResourceList {
