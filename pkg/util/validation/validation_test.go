...
func TestIsValidPodName(t *testing.T) {
	...
	for _, val := range goodValues {
		if msgs := IsValidPodName(val); len(msgs) != 0 {
...
func TestIsValidPortRange(t *testing.T) {
	goodPorts := []int{1, 2, 1000, 16384, 32768, 65534, 65535}
	goodRanges := []int{1, 1, 1, 1000, 55, 2, 1}
	for i := range goodPorts {
		if msgs := IsValidPortRange(goodPorts[i], goodRanges[i]); len(msgs) != 0 {
			t.Errorf("expected true for %d-%d, got %v", goodPorts[i], goodRanges[i], msgs)
		}
	}

	badPorts := []int{0, -1, 65534, 100000}
	badRanges := []int{5000, 0, 3, 1}
	for i := range badPorts {
		if msgs := IsValidPortRange(badPorts[i], badRanges[i]); len(msgs) == 0 {
			t.Errorf("expected false for %d-%d", badPorts[i], badRanges[i])
		}
	}
}
