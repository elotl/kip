...
const podNameFmt string = "[a-z0-9]([-_.a-z0-9]*[a-z0-9])?"
const PodNameMaxLength int = 255

var podNameRegexp = regexp.MustCompile("^" + podNameFmt + "$")

func IsValidPodName(value string) []string {
	var errs []string
	if len(value) > PodNameMaxLength {
		errs = append(errs, MaxLenError(PodNameMaxLength))
	}
	if !podNameRegexp.MatchString(value) {
		errs = append(errs, RegexError(podNameFmt, "namespace/example-pod"))
	}
	return errs
}
...
// IsValidPortRange tests that the argument is a valid, non-zero port number.
func IsValidPortRange(port, rangeSize int) []string {
	if port < 1 || port > 65535 {
		return []string{InclusiveRangeError(1, 65535)}
	}
	if port+rangeSize > 65536 {
		return []string{"Port range must not extend beyond 65535"}
	}
	return nil
}

// IsValidPortNum tests that the argument is a valid, non-zero port number.
func IsValidICMPPortRange(port, rangeSize int) []string {
	if port < -1 || port > 255 {
		return []string{InclusiveRangeError(1, 255)}
	}
	if port+rangeSize > 255 {
		return []string{"ICMP Port range must not extend beyond 255"}
	}
	return nil
}
